# Phân tích Thiết kế Logic UC24 - Phê duyệt phiếu

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Phê duyệt phiếu** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Khởi tạo và thực thi luồng phê duyệt nhiều bước theo loại phiếu, đơn vị và cấp quản lý.
...
- Datasource tham chiếu thực tế: `tbl_flow_pheduyet, tbl_pheduyet_process, tbl_his_pheduyet, tbl_phieu_yeucau`.
- Nhãn giao diện tiêu biểu: Không có màn hình Power Apps trực tiếp.

### 1.3. Actor

- Actor nghiệp vụ: Người đề nghị, quản lý phê duyệt, hệ thống.
...
- Người dùng đã đăng nhập và có quyền `UC24`.
...
- **`[BR-UC24-01]`** Flow active duy nhất theo phạm vi.
- **`[BR-UC24-02]`** Không tự duyệt nếu policy cấm người tạo là người duyệt.
- **`[BR-UC24-03]`** Chỉ người duyệt hiện tại được quyết định.
- **`[BR-UC24-04]`** Một bước chỉ quyết định một lần.
- **`[BR-UC24-05]`** Từ chối kết thúc luồng trừ cấu hình khác.
- **`[BR-UC24-06]`** Lịch sử phê duyệt bất biến.

- **`[BR-UC24-07]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC24-08]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC24-09]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | SP nghiệp vụ khởi tạo yêu cầu duyệt | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Xác định flow và các bước áp dụng | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Gán người duyệt hiện tại | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Người duyệt mở inbox | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Chọn đồng ý/từ chối và nhập ý kiến | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | Ghi lịch sử quyết định | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 7 | Chuyển bước tiếp hoặc kết thúc phiếu | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| NOT_STARTED | Trạng thái nghiệp vụ NOT_STARTED của phê duyệt phiếu | Theo state machine và quyền của SP |
| WAITING | Trạng thái nghiệp vụ WAITING của phê duyệt phiếu | Theo state machine và quyền của SP |
| APPROVED | Trạng thái nghiệp vụ APPROVED của phê duyệt phiếu | Theo state machine và quyền của SP |
| REJECTED | Trạng thái nghiệp vụ REJECTED của phê duyệt phiếu | Theo state machine và quyền của SP |
| CANCELLED | Trạng thái nghiệp vụ CANCELLED của phê duyệt phiếu | Theo state machine và quyền của SP |
...
```typescript
export interface Ph__duy_t_phi_uCommand {
  requestId: string;
  action: string;
...
export interface Ph__duy_t_phi_uResult {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/approval/uc24` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/approval/uc24/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/approval/uc24/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] Ph__duy_t_phi_uCommand request)
{
...
|---|---|---|
| usp_MMS_UC24_StartApproval | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC24_GetApprovalInbox | Read/query | Read-only |
| usp_MMS_UC24_DecideApproval | Command nghiệp vụ | Bắt buộc khi ghi |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC24_DecideApproval
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC24.
        -- 5. Ghi/cập nhật tbl_flow_pheduyet, tbl_pheduyet_process, tbl_his_pheduyet, tbl_phieu_yeucau theo nghiệp vụ.
...
|---|---|---|---|---|---|
| tbl_flow_pheduyet | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_pheduyet_process | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_his_pheduyet | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_phieu_yeucau | - | X | X | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_flow_pheduyet`.
- Dữ liệu liên quan: `tbl_pheduyet_process, tbl_his_pheduyet, tbl_phieu_yeucau`.
...
```mermaid
flowchart TD
    A[Mở màn hình UC24] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> NOT_STARTED
    NOT_STARTED --> WAITING: SP xác nhận chuyển trạng thái
    WAITING --> APPROVED: SP xác nhận chuyển trạng thái
    APPROVED --> REJECTED: SP xác nhận chuyển trạng thái
    REJECTED --> CANCELLED: SP xác nhận chuyển trạng thái
    CANCELLED --> [*]
```
...
|---|---|---|
| AC-UC24-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC24-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC24-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC24-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC24-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC24-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC24-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC24-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC24-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC24-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC24-11 | Audit | Đúng user, time, action, object |
| AC-UC24-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
| Tables/Views | `tbl_flow_pheduyet,tbl_pheduyet_process,tbl_his_pheduyet,tbl_phieu_yeucau` |
| Stored Procedures | `usp_MMS_UC24_StartApproval,usp_MMS_UC24_GetApprovalInbox,usp_MMS_UC24_DecideApproval` |
| React module | `Approval` |
| API base | `/api/v1/approval/uc24` |
