# Phân tích Thiết kế Logic UC23 - Thủ tục xuất kho

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Thủ tục xuất kho** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Xác nhận xuất kho từ hàng đã soạn hoặc xuất trực tiếp, giảm tồn batch và tạo chứng từ giao dịch.

### 1.2. Phạm vi và nguồn hiện hữu

| Màn hình | Nguồn | Datasource phát hiện trong YAML |
|---|---|---|
| scr_xuatkho_thutuc | Quản lý kho vật tư .msapp | tbl_phieu_yeucau_chitiet, vw_xuatkho_chitiet, v_yeucau_soanhang, tbl_phieu_yeucau |
| scr_xuatkho_tructiep | Quản lý kho vật tư .msapp | tbl_phieu_nhan_hang_image, vw_status_phieu_nhanhang, tbl_phieu_nhan_hang, AzureBlobStorage, MMS_sql |
| scr_xuatkho_phieu_print | Quản lý kho vật tư .msapp | vw_xuatkho_chitiet, v_yeucau_soanhang |
| scr_xuatkho_phieu_print_20 | Quản lý kho vật tư .msapp | vw_xuatkho_chitiet, v_yeucau_soanhang |

- Thao tác Power Fx phát hiện: **Collect, Flow.Run, Navigate, Patch, Remove**.
- Datasource tham chiếu thực tế: `AzureBlobStorage, MMS_sql, tbl_phieu_nhan_hang, tbl_phieu_nhan_hang_image, tbl_phieu_yeucau, tbl_phieu_yeucau_chitiet, v_yeucau_soanhang, vw_status_phieu_nhanhang, vw_xuatkho_chitiet`.
- Nhãn giao diện tiêu biểu: `Bộ Phận / Tổ:`, `Tên Bộ phận / Tổ sản xuất`, `SL Yêu Cầu`, `Ghi chú`, `Bộ Phận / Tổ`, `Phiếu ĐNXK`, `Người Đề Nghị`, `Thời Gian Xuất`, `SL Xuất`, `Người Đề Nghị:`, `Tên Người ĐNXK`, `Trạng Thái`.

### 1.3. Actor

- Actor nghiệp vụ: Thủ kho xuất, người nhận.
...
- Người dùng đã đăng nhập và có quyền `UC23`.
...
- **`[BR-UC23-01]`** Không xuất batch vượt tồn.
- **`[BR-UC23-02]`** Chỉ xuất dòng đã soạn trừ luồng trực tiếp có quyền.
- **`[BR-UC23-03]`** Phiếu transaction và transaction phải atomic.
- **`[BR-UC23-04]`** Số lượng batch sau xuất không âm.
- **`[BR-UC23-05]`** Cập nhật trạng thái yêu cầu theo tổng đã xuất.
- **`[BR-UC23-06]`** In phiếu chỉ sau commit thành công.

- **`[BR-UC23-07]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC23-08]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC23-09]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | Chọn phiếu/hàng đã soạn | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Kiểm tra người nhận và chứng từ | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Đối chiếu batch/số lượng | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Xác nhận xuất | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Ghi phiếu transaction | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | Ghi transaction xuất và giảm tồn | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 7 | Cập nhật phiếu yêu cầu | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 8 | In phiếu xuất | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| READY | Trạng thái nghiệp vụ READY của thủ tục xuất kho | Theo state machine và quyền của SP |
| ISSUING | Trạng thái nghiệp vụ ISSUING của thủ tục xuất kho | Theo state machine và quyền của SP |
| ISSUED | Trạng thái nghiệp vụ ISSUED của thủ tục xuất kho | Theo state machine và quyền của SP |
| CANCELLED | Trạng thái nghiệp vụ CANCELLED của thủ tục xuất kho | Theo state machine và quyền của SP |
...
```typescript
export interface Th__t_c_xu_t_khoCommand {
  requestId: string;
  action: string;
...
export interface Th__t_c_xu_t_khoResult {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/outbound/uc23` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/outbound/uc23/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/outbound/uc23/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] Th__t_c_xu_t_khoCommand request)
{
...
|---|---|---|
| usp_MMS_UC23_GetReadyToIssue | Read/query | Read-only |
| usp_MMS_UC23_ConfirmIssue | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC23_DirectIssue | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC23_GetIssuePrint | Read/query | Read-only |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC23_DirectIssue
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC23.
        -- 5. Ghi/cập nhật tbl_batch_inv, tbl_phieu_transaction, tbl_transaction, tbl_phieu_yeucau theo nghiệp vụ.
...
| tbl_phieu_transaction | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_transaction | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_phieu_yeucau | - | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_phieu_yeucau_chitiet | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |
| vw_phieu_xuatkho | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |
| vw_xuatkho_chitiet | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_batch_inv`.
- Dữ liệu liên quan: `tbl_phieu_transaction, tbl_transaction, tbl_phieu_yeucau, tbl_phieu_yeucau_chitiet, vw_phieu_xuatkho, vw_xuatkho_chitiet`.
...
sequenceDiagram
    ...
    User->>UI: Khởi tạo thao tác Thủ tục xuất kho
...
```mermaid
flowchart TD
    A[Mở màn hình UC23] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> READY
    READY --> ISSUING: SP xác nhận chuyển trạng thái
    ISSUING --> ISSUED: SP xác nhận chuyển trạng thái
    ISSUED --> CANCELLED: SP xác nhận chuyển trạng thái
    CANCELLED --> [*]
```
...
|---|---|---|
| AC-UC23-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC23-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC23-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC23-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC23-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC23-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC23-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC23-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC23-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC23-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC23-11 | Audit | Đúng user, time, action, object |
| AC-UC23-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC23 - Thủ tục xuất kho` |
| Power Apps screens | `scr_xuatkho_thutuc,scr_xuatkho_tructiep,scr_xuatkho_phieu_print,scr_xuatkho_phieu_print_20` |
| Power Fx operations | `Collect, Flow.Run, Navigate, Patch, Remove` |
| Tables/Views | `tbl_batch_inv,tbl_phieu_transaction,tbl_transaction,tbl_phieu_yeucau,tbl_phieu_yeucau_chitiet,vw_phieu_xuatkho,vw_xuatkho_chitiet` |
| Stored Procedures | `usp_MMS_UC23_GetReadyToIssue,usp_MMS_UC23_ConfirmIssue,usp_MMS_UC23_DirectIssue,usp_MMS_UC23_GetIssuePrint` |
| React module | `Outbound` |
| API base | `/api/v1/outbound/uc23` |
