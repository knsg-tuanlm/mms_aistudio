...
```typescript
export interface Phi_u_ki_m_QCCommand {
  requestId: string;
  action: string;
...
export interface Phi_u_ki_m_QCResult {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/quality/uc13` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/quality/uc13/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/quality/uc13/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] Phi_u_ki_m_QCCommand request)
{
    ...
    var result = await _sp.ExecuteAsync<dynamic>(
        "usp_MMS_UC13_CreateInspectionSheet",
        new {
...
|---|---|---|
| usp_MMS_UC13_CreateInspectionSheet | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC13_GetInspectionSheet | Read/query | Read-only |
| usp_MMS_UC13_PrintInspectionSheet | Read/query | Read-only |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC13_CreateInspectionSheet
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC13.
        -- 5. Ghi/cập nhật tbl_qc_kiem, tbl_khaibao_qc, vw_phieukiem_group_qc, vw_ketqua_phieu_qc theo nghiệp vụ.
...
| vw_phieukiem_group_qc | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |
...
- Dữ liệu liên quan: `tbl_khaibao_qc, vw_phieukiem_group_qc, vw_ketqua_phieu_qc, vw_ketqua_vattu_qc, tbl_phieu_nhan_hang`.
...
sequenceDiagram
    ...
    User->>UI: Khởi tạo thao tác Phiếu kiểm QC
...
```mermaid
flowchart TD
    A[Mở màn hình UC13] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> IN_PROGRESS: SP xác nhận chuyển trạng thái
    IN_PROGRESS --> PASSED: SP xác nhận chuyển trạng thái
    PASSED --> FAILED: SP xác nhận chuyển trạng thái
    FAILED --> CANCELLED: SP xác nhận chuyển trạng thái
    CANCELLED --> [*]
```
...
|---|---|---|
| AC-UC13-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC13-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC13-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC13-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC13-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC13-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC13-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC13-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC13-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC13-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC13-11 | Audit | Đúng user, time, action, object |
| AC-UC13-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC13 - Phiếu kiểm QC` |
| Power Apps screens | `scr_qc_phieukiem,scr_qc_phieukiem_print,scr_qc_info_danhgia` |
| Power Fx operations | `ClearCollect, Navigate, Patch` |
| Tables/Views | `tbl_qc_kiem,tbl_khaibao_qc,vw_phieukiem_group_qc,vw_ketqua_phieu_qc,vw_ketqua_vattu_qc,tbl_phieu_nhan_hang` |
| Stored Procedures | `usp_MMS_UC13_CreateInspectionSheet,usp_MMS_UC13_GetInspectionSheet,usp_MMS_UC13_PrintInspectionSheet` |
| React module | `Quality` |
| API base | `/api/v1/quality/uc13` |
