# Phân tích Thiết kế Logic UC05 - Nhận hàng không PO

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Nhận hàng không PO** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Ghi nhận vật tư đến kho không có PO với lý do, chứng từ và kiểm soát phê duyệt bắt buộc.

### 1.2. Phạm vi và nguồn hiện hữu

| Màn hình | Nguồn | Datasource phát hiện trong YAML |
|---|---|---|
| scr_nhanhang_khong_po | Kho vật tư .msapp | tbl_phieu_nhan_hang_image, MMS_insert_phieu_nhanhang, tbl_chitiet_nhanhang, tbl_phieu_nhan_hang, AzureBlobStorage, MMS_sql |

- Thao tác Power Fx phát hiện: **Collect, Flow.Run, Patch, Remove**.
- Datasource tham chiếu thực tế: `AzureBlobStorage, MMS_insert_phieu_nhanhang, MMS_sql, tbl_chitiet_nhanhang, tbl_phieu_nhan_hang, tbl_phieu_nhan_hang_image`.
- Nhãn giao diện tiêu biểu: `Xác nhận`, `TẠO PHIẾU`, `Chụp ảnh`, `Vui lòng nhập tên NCC`, `Nhà Cung Cấp:`.

### 1.3. Actor

- Actor nghiệp vụ: Nhân viên nhận hàng, quản lý được cấp quyền.
...
- Người dùng đã đăng nhập và có quyền `UC05`.
...
- **`[BR-UC05-01]`** Lý do không PO là bắt buộc.
- **`[BR-UC05-02]`** Chỉ role được cấp quyền mới tạo phiếu.
- **`[BR-UC05-03]`** Vật tư phải có trong danh mục.
- **`[BR-UC05-04]`** Số lượng phải lớn hơn 0.
- **`[BR-UC05-05]`** Chứng từ bắt buộc theo loại nhận cấu hình.
- **`[BR-UC05-06]`** Phiếu không PO không được giả lập số PO.

- **`[BR-UC05-07]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC05-08]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC05-09]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | Chọn loại nhận không PO | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Nhập nhà cung cấp/nguồn giao | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Chọn vật tư và số lượng | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Nhập lý do không PO | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Đính kèm chứng từ | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | Gửi xác nhận hoặc phê duyệt | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 7 | Sinh phiếu nhận | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| DRAFT | Trạng thái nghiệp vụ DRAFT của nhận hàng không po | Theo state machine và quyền của SP |
| WAITING_APPROVAL | Trạng thái nghiệp vụ WAITING_APPROVAL của nhận hàng không po | Theo state machine và quyền của SP |
| CONFIRMED | Trạng thái nghiệp vụ CONFIRMED của nhận hàng không po | Theo state machine và quyền của SP |
| CANCELLED | Trạng thái nghiệp vụ CANCELLED của nhận hàng không po | Theo state machine và quyền của SP |
...
```typescript
export interface Nh_n_h_ng_kh_ng_POCommand {
  requestId: string;
  action: string;
...
export interface Nh_n_h_ng_kh_ng_POResult {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/receiving/uc05` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/receiving/uc05/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/receiving/uc05/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] Nh_n_h_ng_kh_ng_POCommand request)
{
    ...
    var result = await _sp.ExecuteAsync<dynamic>(
        "usp_MMS_UC05_AddReceiptAttachment",
        new {
...
|---|---|---|
| usp_MMS_UC05_CreateNonPOReceipt | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC05_AddReceiptAttachment | Command nghiệp vụ | Bắt buộc khi ghi |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC05_AddReceiptAttachment
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC05.
        -- 5. Ghi/cập nhật tbl_phieu_nhan_hang, tbl_chitiet_nhanhang, tbl_dm_vattu, tbl_phieu_nhan_hang_image theo nghiệp vụ.
...
| tbl_dm_vattu | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_phieu_nhan_hang_image | - | X | X | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_phieu_nhan_hang`.
- Dữ liệu liên quan: `tbl_chitiet_nhanhang, tbl_dm_vattu, tbl_phieu_nhan_hang_image`.
...
sequenceDiagram
    ...
    User->>UI: Khởi tạo thao tác Nhận hàng không PO
...
```mermaid
flowchart TD
    A[Mở màn hình UC05] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> WAITING_APPROVAL: SP xác nhận chuyển trạng thái
    WAITING_APPROVAL --> CONFIRMED: SP xác nhận chuyển trạng thái
    CONFIRMED --> CANCELLED: SP xác nhận chuyển trạng thái
    CANCELLED --> [*]
```
...
|---|---|---|
| AC-UC05-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC05-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC05-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC05-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC05-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC05-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC05-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC05-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC05-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC05-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC05-11 | Audit | Đúng user, time, action, object |
| AC-UC05-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC05 - Nhận hàng không PO` |
| Power Apps screens | `scr_nhanhang_khong_po` |
| Power Fx operations | `Collect, Flow.Run, Patch, Remove` |
| Tables/Views | `tbl_phieu_nhan_hang,tbl_chitiet_nhanhang,tbl_dm_vattu,tbl_phieu_nhan_hang_image` |
| Stored Procedures | `usp_MMS_UC05_CreateNonPOReceipt,usp_MMS_UC05_AddReceiptAttachment` |
| React module | `Receiving` |
| API base | `/api/v1/receiving/uc05` |
