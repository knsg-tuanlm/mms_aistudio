...
```typescript
export interface T_o____ngh__xu_t_khoCommand {
  requestId: string;
  action: string;
...
export interface T_o____ngh__xu_t_khoResult {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/outbound/uc19` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/outbound/uc19/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/outbound/uc19/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] T_o____ngh__xu_t_khoCommand request)
{
    ...
    var result = await _sp.ExecuteAsync<dynamic>(
        "usp_MMS_UC19_CreateIssueRequest",
        new {
...
|---|---|---|
| usp_MMS_UC19_GetRequestContext | Read/query | Read-only |
| usp_MMS_UC19_CreateIssueRequest | Command nghiệp vụ | Bắt buộc khi ghi |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC19_CreateIssueRequest
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC19.
...
| vw_dinhmuc_conlai | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |
| vw_dm_vattu_tonkho | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_phieu_yeucau`.
- Dữ liệu liên quan: `tbl_phieu_yeucau_chitiet, tbl_dinhmuc, tbl_dm_kehoach, vw_dinhmuc_conlai, vw_dm_vattu_tonkho`.
...
```mermaid
flowchart TD
    A[Mở màn hình UC19] --> B[Tải context qua read SP]
...
|---|---|---|
| AC-UC19-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC19-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC19-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC19-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC19-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC19-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC19-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC19-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC19-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC19-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC19-11 | Audit | Đúng user, time, action, object |
| AC-UC19-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC19 - Tạo đề nghị xuất kho` |
| Power Apps screens | `scr_denghi_xuatkho_request,scr_denghi_xuatkho_planning,scr_denghi_xuatkho_no_planning,scr_denghi_xuatkho_planning_vuot` |
| Power Fx operations | `Collect, Patch, Remove` |
| Tables/Views | `tbl_phieu_yeucau,tbl_phieu_yeucau_chitiet,tbl_dinhmuc,tbl_dm_kehoach,vw_dinhmuc_conlai,vw_dm_vattu_tonkho` |
| Stored Procedures | `usp_MMS_UC19_GetRequestContext,usp_MMS_UC19_CreateIssueRequest` |
| React module | `Outbound` |
| API base | `/api/v1/outbound/uc19` |
