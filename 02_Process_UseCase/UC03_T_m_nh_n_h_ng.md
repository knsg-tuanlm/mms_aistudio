# Phân tích Thiết kế Logic UC03 - Tạm nhận hàng

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Tạm nhận hàng** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Ghi nhận nhanh chứng từ và vật tư đến cổng kho trước khi thực hiện QC/nhập kho chính thức.

### 1.2. Phạm vi và nguồn hiện hữu

| Màn hình | Nguồn | Datasource phát hiện trong YAML |
|---|---|---|
| scr_tam_nhanhang | Quản lý kho vật tư .msapp | tbl_phieu_nhan_hang_image, MMS_insert_phieu_nhanhang, vw_chitiet_ddh_conlai, tbl_chitiet_nhanhang, MMS_insert_nhanhang, tbl_phieu_nhan_hang, AzureBlobStorage, vw_nhanhang_copo, tbl_ChiTietDDH, vw_distinctPO, MMS_sql |
| scr_tam_nhanhang_po | Quản lý kho vật tư .msapp | vw_chitiet_ddh_conlai, vw_nhanhang_copo, tbl_ChiTietDDH |
...
- Nhãn giao diện tiêu biểu: `Ngày giao`, `Ngày đặt`, `Nhà cung cấp`, `Mã PO`, `Mã PO:`, `Nhà cung cấp:`, `Số lượng nhận hàng vượt 10% PO!`, `SL thực nhận`, `SL chứng từ`, `SL đã nhận`, `ĐVT`, `SL Đặt`.

### 1.3. Actor

- Actor nghiệp vụ: Nhân viên nhận hàng, thủ kho.
...
- Người dùng đã đăng nhập và có quyền `UC03`.
...
- **`[BR-UC03-01]`** Số phiếu nhận phải duy nhất.
- **`[BR-UC03-02]`** PO phải tồn tại và còn số lượng nhận.
- **`[BR-UC03-03]`** Số lượng tạm nhận phải lớn hơn 0.
- **`[BR-UC03-04]`** Không vượt số lượng còn lại nếu không có quyền ngoại lệ.
- **`[BR-UC03-05]`** Header và chi tiết được ghi cùng transaction.
- **`[BR-UC03-06]`** Tạm nhận chưa làm tăng tồn kho batch.

- **`[BR-UC03-07]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC03-08]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC03-09]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | Tra cứu PO hoặc chọn nhận không PO | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Nhập thông tin giao nhận | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Chọn các dòng vật tư và số lượng thực nhận | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Đính kèm thông tin chứng từ | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Xác nhận tạm nhận | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | Theo dõi phiếu trong lịch sử | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| DRAFT | Trạng thái nghiệp vụ DRAFT của tạm nhận hàng | Theo state machine và quyền của SP |
| RECEIVED | Trạng thái nghiệp vụ RECEIVED của tạm nhận hàng | Theo state machine và quyền của SP |
| CANCELLED | Trạng thái nghiệp vụ CANCELLED của tạm nhận hàng | Theo state machine và quyền của SP |
...
```typescript
export interface T_m_nh_n_h_ngCommand {
  requestId: string;
  action: string;
...
export interface T_m_nh_n_h_ngResult {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/receiving/uc03` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/receiving/uc03/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/receiving/uc03/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] T_m_nh_n_h_ngCommand request)
{
    ...
    var result = await _sp.ExecuteAsync<dynamic>(
        "usp_MMS_UC03_CreateStagingReceipt",
        new {
...
|---|---|---|
| usp_MMS_UC03_SearchPO | Read/query | Read-only |
| usp_MMS_UC03_CreateStagingReceipt | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC03_GetReceiptHistory | Read/query | Read-only |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC03_CreateStagingReceipt
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC03.
...
| vw_chitietnhan_coPO | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_phieu_nhan_hang`.
- Dữ liệu liên quan: `tbl_chitiet_nhanhang, tbl_ChiTietDDH, vw_distinctPO, vw_chitietnhan_coPO`.
...
sequenceDiagram
    ...
    User->>UI: Khởi tạo thao tác Tạm nhận hàng
...
```mermaid
flowchart TD
    A[Mở màn hình UC03] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> RECEIVED: SP xác nhận chuyển trạng thái
    RECEIVED --> CANCELLED: SP xác nhận chuyển trạng thái
    CANCELLED --> [*]
```
...
|---|---|---|
| AC-UC03-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC03-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC03-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC03-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC03-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC03-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC03-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC03-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC03-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC03-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC03-11 | Audit | Đúng user, time, action, object |
| AC-UC03-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC03 - Tạm nhận hàng` |
| Power Apps screens | `scr_tam_nhanhang,scr_tam_nhanhang_po,scr_tam_nhanhang_log` |
| Power Fx operations | `Collect, Flow.Run, Navigate, Patch, Remove` |
| Tables/Views | `tbl_phieu_nhan_hang,tbl_chitiet_nhanhang,tbl_ChiTietDDH,vw_distinctPO,vw_chitietnhan_coPO` |
| Stored Procedures | `usp_MMS_UC03_SearchPO,usp_MMS_UC03_CreateStagingReceipt,usp_MMS_UC03_GetReceiptHistory` |
| React module | `Receiving` |
| API base | `/api/v1/receiving/uc03` |
