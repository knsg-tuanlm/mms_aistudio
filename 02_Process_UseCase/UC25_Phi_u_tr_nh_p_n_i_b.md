# Phân tích Thiết kế Logic UC25 - Phiếu trả/nhập nội bộ

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Phiếu trả/nhập nội bộ** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Tạo phiếu trả vật tư nội bộ và để thủ kho xác nhận nhập đạt, nhập chờ xử lý hoặc từ chối.

### 1.2. Phạm vi và nguồn hiện hữu

| Màn hình | Nguồn | Datasource phát hiện trong YAML |
|---|---|---|
| scr_phieutra_noibo_ui | Thiết kế mới/chưa có trong hai gói msapp | Không có YAML nguồn |
| scr_thukho_xacnhan_noibo | Thiết kế mới/chưa có trong hai gói msapp | Không có YAML nguồn |

- Thao tác Power Fx phát hiện: **Không có thao tác UI trực tiếp; thiết kế từ bảng/view và use case**.
- Datasource tham chiếu thực tế: `tbl_phieu_nhap_noibo, tbl_chitiet_nhap_noibo, tbl_batch_inv, tbl_phieu_transaction, tbl_transaction, tbl_sx_bravo, tbl_dm_vattu`.
- Nhãn giao diện tiêu biểu: Không có màn hình Power Apps trực tiếp.

### 1.3. Actor

- Actor nghiệp vụ: Đơn vị sản xuất trả, thủ kho.
...
- Người dùng đã đăng nhập và có quyền `UC25`.
...
- **`[BR-UC25-01]`** Phiếu tạo ở status 1.
- **`[BR-UC25-02]`** Từ chối cập nhật status 3 và không tạo batch.
- **`[BR-UC25-03]`** Chấp nhận tạo batch với id_nhanhang cố định 9788.
- **`[BR-UC25-04]`** Nhập đạt dùng trang_thai_ton 1 và nhap_kho 1.
- **`[BR-UC25-05]`** Không đạt dùng trang_thai_ton 3 và nhap_kho 2.
- **`[BR-UC25-06]`** tbl_transaction phải ghi id_phieu_trans.
- **`[BR-UC25-07]`** Toàn bộ xử lý xác nhận atomic.

- **`[BR-UC25-08]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC25-09]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC25-10]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | Đơn vị chọn thông tin phiếu và nhóm lý do | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Thêm vật tư/số lượng/lý do/đơn vị Bravo vào collection | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Xác nhận tạo header và chi tiết | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Thủ kho chọn phiếu chờ xử lý | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Chọn đạt/không đạt/từ chối | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | SP tạo batch và transaction nếu chấp nhận | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 7 | Cập nhật trạng thái phiếu | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| CREATED | Trạng thái nghiệp vụ CREATED của phiếu trả/nhập nội bộ | Theo state machine và quyền của SP |
| WAREHOUSE_ACCEPTED | Trạng thái nghiệp vụ WAREHOUSE_ACCEPTED của phiếu trả/nhập nội bộ | Theo state machine và quyền của SP |
| WAREHOUSE_REJECTED | Trạng thái nghiệp vụ WAREHOUSE_REJECTED của phiếu trả/nhập nội bộ | Theo state machine và quyền của SP |
| WAREHOUSED | Trạng thái nghiệp vụ WAREHOUSED của phiếu trả/nhập nội bộ | Theo state machine và quyền của SP |
...
```typescript
export interface Phi_u_tr__nh_p_n_i_b_Command {
  requestId: string;
  action: string;
...
export interface Phi_u_tr__nh_p_n_i_b_Result {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/internal-return/uc25` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/internal-return/uc25/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/internal-return/uc25/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] Phi_u_tr__nh_p_n_i_b_Command request)
{
...
|---|---|---|
| usp_MMS_UC25_CreateInternalReturn | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC25_GetPendingReturns | Read/query | Read-only |
| usp_xacnhan_phieu_nhap_noibo | Command nghiệp vụ | Bắt buộc khi ghi |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_xacnhan_phieu_nhap_noibo
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC25.
        -- 5. Ghi/cập nhật tbl_phieu_nhap_noibo, tbl_chitiet_nhap_noibo, tbl_batch_inv, tbl_phieu_transaction theo nghiệp vụ.
...
|---|---|---|---|---|---|
| tbl_phieu_nhap_noibo | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_chitiet_nhap_noibo | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
...
| tbl_sx_bravo | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_dm_vattu | - | X | - | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_phieu_nhap_noibo`.
- Dữ liệu liên quan: `tbl_chitiet_nhap_noibo, tbl_batch_inv, tbl_phieu_transaction, tbl_transaction, tbl_sx_bravo, tbl_dm_vattu`.
...
```mermaid
flowchart TD
    A[Mở màn hình UC25] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> CREATED
    CREATED --> WAREHOUSE_ACCEPTED: SP xác nhận chuyển trạng thái
    WAREHOUSE_ACCEPTED --> WAREHOUSE_REJECTED: SP xác nhận chuyển trạng thái
    WAREHOUSE_REJECTED --> WAREHOUSED: SP xác nhận chuyển trạng thái
    WAREHOUSED --> [*]
```
...
|---|---|---|
| AC-UC25-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC25-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC25-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC25-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC25-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC25-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC25-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC25-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC25-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC25-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC25-11 | Audit | Đúng user, time, action, object |
| AC-UC25-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC25 - Phiếu trả/nhập nội bộ` |
| Power Apps screens | `scr_phieutra_noibo_ui,scr_thukho_xacnhan_noibo` |
| Power Fx operations | `Không có thao tác UI trực tiếp; thiết kế từ bảng/view và use case` |
| Tables/Views | `tbl_phieu_nhap_noibo,tbl_chitiet_nhap_noibo,tbl_batch_inv,tbl_phieu_transaction,tbl_transaction,tbl_sx_bravo,tbl_dm_vattu` |
| Stored Procedures | `usp_MMS_UC25_CreateInternalReturn,usp_MMS_UC25_GetPendingReturns,usp_xacnhan_phieu_nhap_noibo` |
| React module | `InternalReturn` |
| API base | `/api/v1/internal-return/uc25` |
