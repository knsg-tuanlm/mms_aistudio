# Phân tích Thiết kế Logic UC11 - Lưu kho lên kệ

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Lưu kho lên kệ** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Gán batch vào vị trí kệ hợp lệ, đổi vị trí và theo dõi sơ đồ lưu kho.

### 1.2. Phạm vi và nguồn hiện hữu

| Màn hình | Nguồn | Datasource phát hiện trong YAML |
|---|---|---|
| scr_luukho | Quản lý kho vật tư .msapp | Không phát hiện datasource trực tiếp |
| scr_luukho_ql | Quản lý kho vật tư .msapp | vw_batch_xuatkho, tbl_transaction |
| scr_luukho_so_do | Quản lý kho vật tư .msapp | tbl_dm_location_ma_tang, tbl_dm_location_ma_ke, tbl_dm_location, tbl_batch_inv |
| scr_luukho_len_ke | Kho vật tư .msapp | MMS_update_location, tbl_batch_inv |
| scr_luukho_doi_ke | Kho vật tư .msapp | MMS_update_location, MMS_update_xuong_ke, tbl_dm_location, tbl_batch_inv |
| scr_luukho_vitri_ke | Kho vật tư .msapp | MMS_update_xuong_ke, tbl_dm_location, tbl_batch_inv |

- Thao tác Power Fx phát hiện: **ClearCollect, Collect, Flow.Run, Navigate, Remove**.
- Datasource tham chiếu thực tế: `MMS_update_location, MMS_update_xuong_ke, tbl_batch_inv, tbl_dm_location, tbl_dm_location_ma_ke, tbl_dm_location_ma_tang, tbl_transaction, vw_batch_xuatkho`.
- Nhãn giao diện tiêu biểu: `Tên Vật Tư:`, `Tên Vật Tư`, `Số Lượng`, `Ngày Nhập Kho`, `ĐVT`, `Vị Trí`, `Tìm kiếm Tên Vật Tư`, `Phiếu`, `Nghiệp Vụ`, `Thời Gian`, `Mã Vật Tư:`, `Tìm kiếm Mã Vật Tư`.

### 1.3. Actor

- Actor nghiệp vụ: Thủ kho lưu kho.
...
- Người dùng đã đăng nhập và có quyền `UC11`.
...
- **`[BR-UC11-01]`** Batch phải tồn tại và chưa xuất hết.
- **`[BR-UC11-02]`** Vị trí phải hoạt động và đúng kho.
- **`[BR-UC11-03]`** Không vượt sức chứa nếu có cấu hình.
- **`[BR-UC11-04]`** Một batch chỉ có một vị trí hiện hành.
- **`[BR-UC11-05]`** Đổi kệ phải lưu vị trí cũ/mới.
- **`[BR-UC11-06]`** Cập nhật batch và lịch sử vị trí atomic.

- **`[BR-UC11-07]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC11-08]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC11-09]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | Quét/chọn batch cần lưu | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Chọn khu/kệ/tầng/vị trí | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Kiểm tra sức chứa và trạng thái vị trí | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Xác nhận đưa lên kệ | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Cập nhật location batch | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | Khi cần chọn batch và vị trí mới để đổi kệ | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| UNASSIGNED | Trạng thái nghiệp vụ UNASSIGNED của lưu kho lên kệ | Theo state machine và quyền của SP |
| STORED | Trạng thái nghiệp vụ STORED của lưu kho lên kệ | Theo state machine và quyền của SP |
| MOVING | Trạng thái nghiệp vụ MOVING của lưu kho lên kệ | Theo state machine và quyền của SP |
| BLOCKED | Trạng thái nghiệp vụ BLOCKED của lưu kho lên kệ | Theo state machine và quyền của SP |
...
```typescript
export interface L_u_kho_l_n_k_Command {
  requestId: string;
  action: string;
...
export interface L_u_kho_l_n_k_Result {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/putaway/uc11` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/putaway/uc11/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/putaway/uc11/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] L_u_kho_l_n_k_Command request)
{
...
|---|---|---|
| usp_MMS_UC11_GetStorageMap | Read/query | Read-only |
| usp_MMS_UC11_PutawayBatch | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC11_MoveBatch | Command nghiệp vụ | Bắt buộc khi ghi |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC11_MoveBatch
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC11.
        -- 5. Ghi/cập nhật tbl_batch_inv, tbl_dm_location, tbl_dm_location_ma_ke, tbl_dm_location_ma_tang theo nghiệp vụ.
...
| tbl_dm_location_ma_ke | X | X | X | - | Nguồn dữ liệu/chứng từ của use case |
| tbl_dm_location_ma_tang | - | X | X | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_batch_inv`.
- Dữ liệu liên quan: `tbl_dm_location, tbl_dm_location_ma_ke, tbl_dm_location_ma_tang`.
...
```mermaid
flowchart TD
    A[Mở màn hình UC11] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> UNASSIGNED
    UNASSIGNED --> STORED: SP xác nhận chuyển trạng thái
    STORED --> MOVING: SP xác nhận chuyển trạng thái
    MOVING --> BLOCKED: SP xác nhận chuyển trạng thái
    BLOCKED --> [*]
```
...
|---|---|---|
| AC-UC11-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC11-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC11-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC11-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC11-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC11-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC11-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC11-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC11-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC11-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC11-11 | Audit | Đúng user, time, action, object |
| AC-UC11-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC11 - Lưu kho lên kệ` |
| Power Apps screens | `scr_luukho,scr_luukho_ql,scr_luukho_so_do,scr_luukho_len_ke,scr_luukho_doi_ke,scr_luukho_vitri_ke` |
| Power Fx operations | `ClearCollect, Collect, Flow.Run, Navigate, Remove` |
| Tables/Views | `tbl_batch_inv,tbl_dm_location,tbl_dm_location_ma_ke,tbl_dm_location_ma_tang` |
| Stored Procedures | `usp_MMS_UC11_GetStorageMap,usp_MMS_UC11_PutawayBatch,usp_MMS_UC11_MoveBatch` |
| React module | `Putaway` |
| API base | `/api/v1/putaway/uc11` |
