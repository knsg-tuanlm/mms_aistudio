# Phân tích Thiết kế Logic UC15 - Khai báo tồn kho

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Khai báo tồn kho** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Khai báo tồn ban đầu/điều chỉnh được kiểm soát theo vật tư, batch, kho, trạng thái tồn và vị trí.

### 1.2. Phạm vi và nguồn hiện hữu

| Màn hình | Nguồn | Datasource phát hiện trong YAML |
|---|---|---|
| scr_tonkho_khaibao | Kho vật tư .msapp | vw_batch_print_tonkho, MMS_insert_tonkho, tbl_transaction, tbl_batch_inv, tbl_dm_vattu |
| scr_tonkho_khaibao_ver1 | Kho vật tư .msapp | vw_batch_print_tonkho, MMS_insert_tonkho, tbl_transaction, tbl_batch_inv, tbl_dm_vattu |

- Thao tác Power Fx phát hiện: **Collect, Flow.Run, Navigate, Patch, Remove**.
- Datasource tham chiếu thực tế: `MMS_insert_tonkho, tbl_batch_inv, tbl_dm_vattu, tbl_transaction, vw_batch_print_tonkho`.
- Nhãn giao diện tiêu biểu: `Tên vật tư:`, `Tìm kiếm tên vật tư`, `Mã vật tư:`, `Tìm kiếm mã vật tư`, `Mã vật tư`, `Tên vật tư`, `ĐVT`, `Số Lượng`, `IN TEM`, `XÁC NHẬN`, `SL TỒN`.

### 1.3. Actor

- Actor nghiệp vụ: Thủ kho, Inventory Admin.
...
- Người dùng đã đăng nhập và có quyền `UC15`.
...
- **`[BR-UC15-01]`** Batch không được trùng trong cùng kho.
- **`[BR-UC15-02]`** Số lượng phải dương.
- **`[BR-UC15-03]`** Vật tư/đơn vị phải hợp lệ.
- **`[BR-UC15-04]`** Khai báo tồn phải sinh chứng từ và transaction.
- **`[BR-UC15-05]`** Không ghi trực tiếp batch nếu thiếu transaction.
- **`[BR-UC15-06]`** Chỉ role được cấp quyền mới được post.

- **`[BR-UC15-07]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC15-08]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC15-09]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | Chọn loại khai báo | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Nhập/quét vật tư và batch | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Nhập số lượng, đơn vị, trạng thái, vị trí | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Kiểm tra trùng và danh mục | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Xem trước chênh lệch | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | Xác nhận ghi tồn và transaction | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| DRAFT | Trạng thái nghiệp vụ DRAFT của khai báo tồn kho | Theo state machine và quyền của SP |
| VALIDATED | Trạng thái nghiệp vụ VALIDATED của khai báo tồn kho | Theo state machine và quyền của SP |
| POSTED | Trạng thái nghiệp vụ POSTED của khai báo tồn kho | Theo state machine và quyền của SP |
| CANCELLED | Trạng thái nghiệp vụ CANCELLED của khai báo tồn kho | Theo state machine và quyền của SP |
...
```typescript
export interface Khai_b_o_t_n_khoCommand {
  requestId: string;
  action: string;
...
export interface Khai_b_o_t_n_khoResult {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/inventory/uc15` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/inventory/uc15/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/inventory/uc15/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] Khai_b_o_t_n_khoCommand request)
{
    ...
    var result = await _sp.ExecuteAsync<dynamic>(
        "usp_MMS_UC15_CreateOpeningStock",
        new {
...
|---|---|---|
| usp_MMS_UC15_ValidateOpeningStock | Command nghiệp vụ | Bắt buộc khi ghi |
| usp_MMS_UC15_CreateOpeningStock | Command nghiệp vụ | Bắt buộc khi ghi |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC15_CreateOpeningStock
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC15.
        -- 5. Ghi/cập nhật tbl_batch_inv, tbl_dm_vattu, tbl_transaction, tbl_phieu_transaction theo nghiệp vụ.
...
- Dữ liệu liên quan: `tbl_dm_vattu, tbl_transaction, tbl_phieu_transaction, v_ton_he_thong`.
...
sequenceDiagram
    ...
    User->>UI: Khởi tạo thao tác Khai báo tồn kho
...
```mermaid
flowchart TD
    A[Mở màn hình UC15] --> B[Tải context qua read SP]
...
stateDiagram-v2
    [*] --> DRAFT
    DRAFT --> VALIDATED: SP xác nhận chuyển trạng thái
    VALIDATED --> POSTED: SP xác nhận chuyển trạng thái
    POSTED --> CANCELLED: SP xác nhận chuyển trạng thái
    CANCELLED --> [*]
```
...
|---|---|---|
| AC-UC15-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC15-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC15-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC15-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC15-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC15-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC15-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC15-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC15-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC15-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC15-11 | Audit | Đúng user, time, action, object |
| AC-UC15-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC15 - Khai báo tồn kho` |
| Power Apps screens | `scr_tonkho_khaibao,scr_tonkho_khaibao_ver1` |
| Power Fx operations | `Collect, Flow.Run, Navigate, Patch, Remove` |
| Tables/Views | `tbl_batch_inv,tbl_dm_vattu,tbl_transaction,tbl_phieu_transaction,v_ton_he_thong` |
| Stored Procedures | `usp_MMS_UC15_ValidateOpeningStock,usp_MMS_UC15_CreateOpeningStock` |
| React module | `Inventory` |
| API base | `/api/v1/inventory/uc15` |
