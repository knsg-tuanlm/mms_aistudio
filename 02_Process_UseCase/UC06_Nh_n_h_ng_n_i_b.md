# Phân tích Thiết kế Logic UC06 - Nhận hàng nội bộ

Tài liệu phân tích toàn diện **Business Logic**, **UI/UX Guidelines**, **Programming Logic**, **Data Logic** và **Diagrams** cho chức năng **Nhận hàng nội bộ** khi chuyển hệ thống Quản lý Kho Vật tư từ Power Apps sang React.
...
Ghi nhận vật tư luân chuyển từ đơn vị nội bộ/đơn vị sản xuất vào quy trình nhận hàng kho.

### 1.2. Phạm vi và nguồn hiện hữu

| Màn hình | Nguồn | Datasource phát hiện trong YAML |
|---|---|---|
| scr_nhanhang_noibo | Kho vật tư .msapp | tbl_phieu_nhan_hang_image, MMS_insert_phieu_nhanhang, MMS_insert_nhanhang, tbl_phieu_nhan_hang, AzureBlobStorage, tbl_dm_vattu, tbl_sx_bravo, MMS_sql |

- Thao tác Power Fx phát hiện: **Collect, Flow.Run, Patch, Remove**.
- Datasource tham chiếu thực tế: `AzureBlobStorage, MMS_insert_nhanhang, MMS_insert_phieu_nhanhang, MMS_sql, tbl_dm_vattu, tbl_phieu_nhan_hang, tbl_phieu_nhan_hang_image, tbl_sx_bravo`.
- Nhãn giao diện tiêu biểu: `Tên vật tư:`, `Tìm kiếm tên vật tư`, `Mã vật tư:`, `Tìm kiếm mã vật tư`, `Mã Vật Tư`, `Tên Vật Tư`, `ĐVT`, `Số Lượng Nhận`, `XÁC NHẬN`, `TẠO PHIẾU`, `Chụp ảnh`, `Đơn Vị Tổ:`.

### 1.3. Actor

- Actor nghiệp vụ: Nhân viên kho, đơn vị giao nội bộ.
...
- Người dùng đã đăng nhập và có quyền `UC06`.
...
- **`[BR-UC06-01]`** Đơn vị Bravo phải đang hoạt động.
- **`[BR-UC06-02]`** Mỗi dòng vật tư chỉ xuất hiện một lần hoặc được SP gộp.
- **`[BR-UC06-03]`** Số lượng phải lớn hơn 0.
- **`[BR-UC06-04]`** Không cập nhật tồn ở bước nhận hàng.
- **`[BR-UC06-05]`** Header và lines phải atomic.
- **`[BR-UC06-06]`** Người tạo lấy từ token.

- **`[BR-UC06-07]`** User thao tác lấy từ token; SP kiểm tra lại quyền và phạm vi dữ liệu.
- **`[BR-UC06-08]`** Mọi command phải hỗ trợ `RequestId` để chống gửi lặp.
- **`[BR-UC06-09]`** Không dùng SQL động do client truyền và không cho backend cập nhật bảng trực tiếp.

### 1.7. Luồng chính

| Bước | Thao tác | React/API | SQL Stored Procedure |
|---|---|---|---|
| 1 | Chọn đơn vị Bravo giao hàng | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 2 | Nhập thông tin phiếu nội bộ | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 3 | Chọn vật tư | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 4 | Nhập số lượng và ghi chú | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 5 | Thêm vào danh sách tạm | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
| 6 | Xác nhận tạo phiếu nhận nội bộ | Hiển thị/thu thập dữ liệu và gọi API | Đọc, kiểm tra rule hoặc ghi dữ liệu trong SP |
...
|---|---|---|
| DRAFT | Trạng thái nghiệp vụ DRAFT của nhận hàng nội bộ | Theo state machine và quyền của SP |
| CONFIRMED | Trạng thái nghiệp vụ CONFIRMED của nhận hàng nội bộ | Theo state machine và quyền của SP |
| QC_PENDING | Trạng thái nghiệp vụ QC_PENDING của nhận hàng nội bộ | Theo state machine và quyền của SP |
| CANCELLED | Trạng thái nghiệp vụ CANCELLED của nhận hàng nội bộ | Theo state machine và quyền của SP |
...
```typescript
export interface Nh_n_h_ng_n_i_b_Command {
  requestId: string;
  action: string;
...
export interface Nh_n_h_ng_n_i_b_Result {
  resultCode: string;
  message: string;
  documentId?: number | string;
  status: string;
...
|---|---|---|
| `GET` | `/api/v1/receiving/uc06` | Danh sách có filter và phân trang |
| `GET` | `/api/v1/receiving/uc06/{id}` | Chi tiết chứng từ/đối tượng |
| `POST` | `/api/v1/receiving/uc06/commands` | Thực thi command qua SP |

### 3.4. Backend handler mỏng

```csharp
[HttpPost("commands")]
public async Task<IActionResult> Execute([FromBody] Nh_n_h_ng_n_i_b_Command request)
{
    ...
    var result = await _sp.ExecuteAsync<dynamic>(
        "usp_MMS_UC06_CreateInternalReceipt",
        new {
...
|---|---|---|
| usp_MMS_UC06_GetBravoUnits | Read/query | Read-only |
| usp_MMS_UC06_CreateInternalReceipt | Command nghiệp vụ | Bắt buộc khi ghi |
...
```sql
CREATE OR ALTER PROCEDURE dbo.usp_MMS_UC06_CreateInternalReceipt
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- 4. Kiểm tra state transition và các BR của UC06.
        -- 5. Ghi/cập nhật tbl_phieu_nhan_hang, tbl_chitiet_nhanhang, tbl_dm_vattu, tbl_sx_bravo theo nghiệp vụ.
...
| tbl_sx_bravo | - | X | X | - | Nguồn dữ liệu/chứng từ của use case |

### 5.2. Quan hệ dữ liệu trọng tâm

- Aggregate gốc: `tbl_phieu_nhan_hang`.
- Dữ liệu liên quan: `tbl_chitiet_nhanhang, tbl_dm_vattu, tbl_sx_bravo`.
...
sequenceDiagram
    ...
    User->>UI: Khởi tạo thao tác Nhận hàng nội bộ
...
```mermaid
flowchart TD
    A[Mở màn hình UC06] --> B[Tải context qua read SP]
...
|---|---|---|
| AC-UC06-01 | Luồng chính với dữ liệu hợp lệ | SP commit, trả SUCCESS và trạng thái đúng |
| AC-UC06-02 | Thiếu trường bắt buộc | INVALID_INPUT; không ghi dữ liệu |
| AC-UC06-03 | User không có quyền | HTTP 403; không lộ dữ liệu |
| AC-UC06-04 | Đối tượng không tồn tại | NOT_FOUND; UI tải lại danh sách |
| AC-UC06-05 | Sai trạng thái nghiệp vụ | INVALID_STATE; transaction rollback |
| AC-UC06-06 | Hai người cập nhật đồng thời | Một thành công, một conflict |
| AC-UC06-07 | Gửi lại cùng RequestId | Không tạo giao dịch trùng |
| AC-UC06-08 | Lỗi ở bước ghi cuối | Rollback toàn bộ bảng |
| AC-UC06-09 | Danh sách lớn | Paging/filter/sort tại SQL |
| AC-UC06-10 | Mobile viewport | Không tràn/đè UI |
| AC-UC06-11 | Audit | Đúng user, time, action, object |
| AC-UC06-12 | Kiểm tra sau commit | Header, lines, trạng thái nhất quán |
...
|---|---|
| Use case | `UC06 - Nhận hàng nội bộ` |
| Power Apps screens | `scr_nhanhang_noibo` |
| Power Fx operations | `Collect, Flow.Run, Patch, Remove` |
| Tables/Views | `tbl_phieu_nhan_hang,tbl_chitiet_nhanhang,tbl_dm_vattu,tbl_sx_bravo` |
| Stored Procedures | `usp_MMS_UC06_GetBravoUnits,usp_MMS_UC06_CreateInternalReceipt` |
| React module | `Receiving` |
| API base | `/api/v1/receiving/uc06` |
